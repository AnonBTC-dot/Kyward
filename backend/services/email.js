...
/**
 * Encrypt a PDF with password using muhammara
 */
async function encryptPdfWithPassword(pdfBuffer, password) {
  try {
    ...
    console.error('⚠️ PDF encryption failed, sending unencrypted:', error.message);
...
async function sendEmailWithPdfAttachment(toEmail, pdfPassword, score, pdfBuffer) {
  ...
  // Try to encrypt the PDF with password
  console.log('🔐 Encrypting PDF with password...');
  const encryptedPdf = await encryptPdfWithPassword(pdfBuffer, pdfPassword);
  const isEncrypted = encryptedPdf !== pdfBuffer;
  console.log(isEncrypted ? '✅ PDF encrypted successfully' : '⚠️ PDF sent without encryption');
...
<body>
  <div class="container">
    ...
    <div class="body">
      <h2>Your Security Plan is Attached</h2>
      <p>Your complete Bitcoin Security & Inheritance Plan (Score: ${score}/100) is attached to this email${isEncrypted ? ' as a password-protected PDF' : ''}.</p>

      <div class="password-box">
        <p style="margin: 0 0 12px 0; color: #666;">${isEncrypted ? 'PDF Password:' : 'Your Account Password Reference:'}</p>
        <div class="password">${pdfPassword}</div>
        <p style="margin: 12px 0 0 0; color: #888; font-size: 13px;">${isEncrypted ? 'You need this password to open the PDF' : 'Save this for your records'}</p>
      </div>
